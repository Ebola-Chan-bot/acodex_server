...
pub async fn create_terminal(
    ...
    // Background child waiter — signals when process exits
    {
        ...
        spawn_blocking(move || {
            ...
            *exit_status.lock().unwrap() = Some(success);
            ...
            tracing::info!("Background child waiter exited for PID {} (success={})", pid, success);
        });
    }
    ...
    Json(serde_json::json!({ "pid": pid })).into_response()
}
...
async fn handle_socket(socket: WebSocket, pid: u32, sessions: Sessions) {
    ...
    if let Some(success) = already_exited {
        let exit_message = ProcessExitMessage { exit_code: Some(if success { 0 } else { 1 }), signal: None, message: String::new() };
    ...
    loop {
        tokio::select! {
            ...
            _ = exit_notify.notified() => {
                ...
                let exit_message = { let success = exit_status_arc.lock().unwrap().unwrap_or(false); ProcessExitMessage { exit_code: Some(if success { 0 } else { 1 }), signal: None, message: String::new() } };
                let exit_json = serde_json::to_string(&exit_message).unwrap_or_default();
                let _ = sender
                    .send(Message::Text(
                        format!("{{\"type\":\"exit\",\"data\":{exit_json}}}").into(),
